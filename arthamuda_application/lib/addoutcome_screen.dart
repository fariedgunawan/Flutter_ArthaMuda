...
class _AddOutcomeScreenState extends State<AddOutcomeScreen> {
  ...
  Future<void> _loadTokenAndData() async {
    ...
    _fetchOutcomeTransactions();
  }

  Future<void> _fetchOutcomeTransactions() async {
    if (_token == null) return;

    try {
      final response = await http.get(
        Uri.parse('http://192.168.200.234:3000/api/transactions/outcome'),
  ...
  try {
    ...
    if (responseData['success'] == true &&
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Outcome data successfully added!'),
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(responseData['message'] ?? 'Failed to add outcome'),
            backgroundColor: _errorColor,
          ),
        );
      }
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('An error occurred while adding outcome'),
          backgroundColor: _errorColor,
        ),
      );
    }
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          ...
                          GestureDetector(
                            onTap: () {
                              Navigator.pushNamed(context, '/listOutcomeTransaction');
                            },
                            child: const Text(
                              'See More',
                              style: TextStyle(
